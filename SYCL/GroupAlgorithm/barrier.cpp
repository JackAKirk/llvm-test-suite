    ...
    q.submit([&](handler &cgh) {
      ...
      cgh.parallel_for(
          nd_range<1>(N, N), [=](nd_item<1> item) {
            ...
            item.async_work_group_copy(loc.get_pointer(), data_acc.get_pointer(), N);
            ...
            item.async_work_group_copy(data_acc.get_pointer(), loc.get_pointer(), N);
