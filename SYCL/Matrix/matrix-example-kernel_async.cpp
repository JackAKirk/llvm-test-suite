// This example additionally uses the asynchronous copy feature from global to
// shared memory that was introduced in sm_80. This example makes use of the
// sub-group/work-group async copy function introduced in the test extension as
// implemented in https://github.com/intel/llvm/pull/4907. Although the memory
// layout is specifically chosen to avoid warp entanglement performance
// penalties and ensure that each memory element is only copied once, the
// addition of this async memory copy route does not lead to a performance
// advantage for the current example. Further investigation into the performance
// of the async copy route is needed in order to correctly design an async copy
// API that is generally effective and can fit into the SYCL standard.
  ...
  q.submit([&](handler &cgh) {
    ...
    cgh.parallel_for<KernelName<T1, T2, M, K, N>>(
        nd_range<2>(GlobalRange, LocalRange), [=](nd_item<2> item) {
          ...
          // These events are needed in the draft proposal that aimed to fit
          // into the OpenCL based SYCL standard.
          __ocl_event_t Event;
          sycl::ext::oneapi::experimental::async_copy_event<sycl::group<2>>
              Ev_group(Event);
          sycl::ext::oneapi::experimental::async_copy_event<
              sycl::ext::oneapi::sub_group>
              Ev(Event);

// Each sub-group (warp) performs two async copy commits. Every work-item in
// each sub-group copies a single element per commit.
#pragma unroll
          for (int m_c = 0; m_c < 2; m_c++) {
            Ev = sycl::ext::oneapi::experimental::joint_async_copy(
                sg,
                accC.get_pointer() +
                    ((group_id_m * Sub_Tiles_M * M) +
                     (m * M + (m_c * Sub_Tiles_N + n))) *
                        Big_N +
                    group_id_n * Sub_Tiles_N * N,
                LocalC.get_pointer() +
                    (m * M + (m_c * Sub_Tiles_N + n)) * Sub_Tiles_N * N,
                Sub_Tiles_N * N);
          }
          ...
          // Wait for the asynchronous copies to complete and synchronize the
          // work-group.
